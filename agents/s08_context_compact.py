...
def run_read(path: str, limit: int | None = None, start: int | None = None) -> str:
    try:
        if limit is None and start is not None:
            limit = start
...
def agent_loop(messages: list):
    ...
    while True:
        ...
        try:
            response = client.chat.completions.create(model=MODEL, system=SYSTEM, messages=messages, tools=TOOLS, max_tokens=8000)
