...
client = create_openai_adapter(api_key=os.environ["API_KEY"], base_url=os.environ["BASE_URL"])
...
def _recent_user_dialogue(messages: list, max_user_turns: int = 3) -> str:
    """Collect recent user-authored turns for memory extraction."""
    user_texts = []
    for msg in reversed(messages):
        if msg.get("role") != "user":
            continue
...
def extract_memories(messages: list):
    ...
    prompt = (
        "Extract user preferences, constraints, or project facts from the user dialogue below.\n"
        "Treat user-authored text as the primary signal.\n"
        "Do not infer memories from assistant/tool chatter unless it is necessary context for a user preference.\n"
...
def spawn_subagent(task: str) -> str:
    ...
    for _ in range(30):
        response = client.chat.completions.create(model=MODEL, system=SUB_SYSTEM,
...
if __name__ == "__main__":
    ...
    print("Enter a question and press Enter. Type q to quit.\n")
    history = []
    while True:
